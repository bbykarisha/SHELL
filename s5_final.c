...
int
simple_pipe(int *fd, Command *command, int index, int count)
{
    int exit_status = 1;

    if (fork() == 0) {
        // перенаправляем ввод всех команд кроме первой
        if (index != 0) {
            dup2(fd[2 * (index - 1)], STDIN_FILENO);
        }

        // перенаправляем вывод всех команд кроме последней
        if (index != command->pipeline_size - 1) {
            dup2(fd[2 * index + 1], STDOUT_FILENO);
        }
        ...
        exit_status = run_command(&command->pipeline_commands[index]); 
        exit(exit_status);
    } 
...
pipeline_command(Command *command)
{
    ...
    if (fork() == 0) {
        switch (command->pipeline_size)
        {
            case 1:
                exit_status = run_command(command->pipeline_commands);
                break;
            default:
            {
                int size = command->pipeline_size - 1; // количество пайпов
                int *multiple_fd = calloc(2 * size, sizeof *multiple_fd);

                for (int i = 0; i < size; ++i) {
                    pipe(multiple_fd + 2 * i);
                }

                for (int i = 0; i <= size; ++i) {
                    exit_status = simple_pipe(multiple_fd, command, i, 2 * size);
                }
